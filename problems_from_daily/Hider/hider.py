...
class Encryption(object):
    ...
    def __init__(self, path, threshold=150):
        ...
        # 打开二值化处理之后的图片
        self.img = self.__openBinaryImage()
        # 计算此图片能存储的最大数据，以及要预留多大空间来存储文件长度
        self.max_size, self.info_length = self.__calculateSize()
       
    # 打印出图片可以存储的最大文件大小
    def __str__(self):
        return '此图片最多可以存储%.2fKB的文件'%(((self.max_size-self.info_length)//8)/1024)
    ...
    # 打开二值化之后的图片
    def __openBinaryImage(self):
        img = Image.open(self.__img_path + self.__img_name + '_binary' + self.__img_suffix)
        table = list(map(lambda i:0 if i < self.__threshold else 1, list(range(256))))
        img = img.convert('L')
        img = img.point(table, '1')
        return img
    ...
    def encrypt(self, file, path):
        ...
        if len(info) > self.max_size:
            raise ValueError('文件过大，此图片最多可以存储%.2fKB的文件'%(((self.max_size-self.info_length)//8)/1024))
        pixels = self.img.load()
        index = 0
        try:
...
if __name__ == '__main__':
    # 初始化加密类，传入原始图片
    e = Encryption('./image_demos/demo.jpg')
    # 打印存储空间
    print(e)
    # 加密文件一，是一个txt
    e.encrypt('./image_demos/need_encrypt_info_1.txt', './image_demos/demo_encrypted_1.jpg')
    # 加密文件二，是一个png
    e.encrypt('./image_demos/need_encrypt_info_2.png', './image_demos/demo_encrypted_2.jpg')
    
    # 初始化解密类，传入需要解密的图片
    d = Decryption('./image_demos/demo_encrypted_1.jpg')
    # 解密文件一，是一个txtx
    d.decrypt('./image_demos/decrypted_info_1.txt')
    
    # 初始化解密类，传入需要解密的图片
    d = Decryption('./image_demos/demo_encrypted_2.jpg')
    # 解密文件二，是一个png
    d.decrypt('./image_demos/decrypted_info_2.png')
